...
CREATE OR REPLACE FUNCTION sqids.isBlockedId(id TEXT, blocklist TEXT[]) RETURNS BOOLEAN AS $$
DECLARE
  lowercase_id TEXT := LOWER(id);
  word TEXT;
BEGIN
  IF blocklist IS NULL THEN
    RETURN FALSE;
  END IF;

  FOREACH word IN ARRAY blocklist LOOP
    word := LOWER(word);
...
DECLARE
  words TEXT[];
BEGIN
  SELECT COALESCE(array_agg(str), ARRAY[]::TEXT[]) INTO words FROM sqids.blocklist;
  RETURN sqids.isBlockedId(id, words);
END;
...
-- Internal: alphabet must already be shuffled (as encode / encodeImmutable do).
-- Prefer those wrappers; do not call this from generated columns.
CREATE OR REPLACE FUNCTION sqids.encodeNumbers(numbers BIGINT[], alphabet TEXT, minLength INT, increment INT, blocklist TEXT[]) RETURNS TEXT AS $$
DECLARE
  offset_var INT;
  arr_alphabet TEXT[];
...
BEGIN
  ...
  FOR i IN 1..array_length(numbers, 1) LOOP
    IF numbers[i] IS NULL OR numbers[i] < 0 THEN
      RAISE EXCEPTION 'Sqids: numbers must be non-negative integers, received %', numbers[i];
    END IF;
  END LOOP;
  ...
  IF sqids.isBlockedId(id, blocklist) THEN
    --raise notice 'Blocked ID: %', id;
    id := sqids.encodeNumbers(numbers, original_alphabet, minLength, increment + 1, blocklist);
  END IF;
...
BEGIN
  SELECT COALESCE(array_agg(str), ARRAY[]::TEXT[]) INTO words FROM sqids.blocklist;
  RETURN sqids.encodeNumbers(numbers, alphabet, minLength, increment, words);
END;
$$ LANGUAGE plpgsql STABLE;
...
CREATE OR REPLACE FUNCTION sqids.encodeImmutable(numbers BIGINT[], alphabet TEXT, minLength INT, blocklist TEXT[]) RETURNS TEXT AS $$
DECLARE
  id TEXT;
BEGIN
  PERFORM sqids.checkAlphabet(alphabet);
...
BEGIN
  ...
  INSERT INTO sqids.blocklist (str)
  SELECT unnest(sqids.defaultBlocklistWords());
END;
$$ LANGUAGE plpgsql;
